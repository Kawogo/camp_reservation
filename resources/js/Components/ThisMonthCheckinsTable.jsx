...
const ThisMonthCheckinsTable = ({ checkins }) => {
    const [checkin, setCheckin] = useState(null);

    // CRUD
    const deleteCheckin = () => {
        router.delete(route("checkins.destroy", checkin));
    };
    ...
    return (
        <div className="mx-auto max-w-screen-xl px-2 lg:px-2">
            <div className="bg-white dark:bg-gray-800 relative border sm:rounded-lg overflow-hidden">
                <div className="flex flex-col md:flex-row items-center justify-between space-y-3 md:space-y-0 md:space-x-4 p-4">
                    <div className="w-full md:w-auto flex flex-col md:flex-row space-y-2 md:space-y-0 items-stretch md:items-center justify-end md:space-x-3 flex-shrink-0">
                        <Link
                            href={route("checkins.create")}
                            className="flex items-center justify-center text-white bg-primary-700 hover:bg-primary-800 focus:ring-4 focus:ring-primary-300 font-medium rounded-sm text-xs px-4 py-1 dark:bg-primary-600 dark:hover:bg-primary-700 focus:outline-none dark:focus:ring-primary-800"
                        >
                            Add New Checkin
                        </Link>
                    </div>
                                        ...
                                        <td className="px-4 py-3">
                                            ...
                                            >
                                                {checkin.status[0].toUpperCase() +
                                                    checkin.status.substring(1)}
                                            </span>
                                        </td>

                                        <td className="px-4 py-3 flex items-center justify-end">
                                            <button
                                                id={`${checkin.id}-dropdown-button`}
                                                data-dropdown-toggle={`table-dropdown-${checkin.id}`}
                                            ...
                                            <div
                                                id={`table-dropdown-${checkin.id}`}
                                                className="hidden z-10 w-44 bg-white rounded divide-y divide-gray-100 shadow dark:bg-gray-700 dark:divide-gray-600"
                                            >
                                                <ul
                                                    className="py-1 text-sm text-gray-700 dark:text-gray-200"
                                                    aria-labelledby={`${checkin.id}-dropdown-button`}
                                                >
                                                    <li>
                                                        <Link
                                                            href={route(
                        ...
                        <div class="relative p-4 w-full max-w-md h-full md:h-auto">
                            <div class="relative p-4 text-center bg-white rounded-lg shadow dark:bg-gray-800 sm:p-5">
                                ...
                                <p class="mb-4 text-gray-500 dark:text-gray-300">
                                    Deleting this also deletes all data related
                                    to this checkin on the system.
                                </p>
...
export default ThisMonthCheckinsTable;
