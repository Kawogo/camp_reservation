...
const EditCamp = ({ auth, camp, companies }) => {
    console.log(camp);
    const { data, setData, put, processing, errors, reset } = useForm({
        name: camp.name,
        company_id: camp.company_id,
    });
    const companiesOpts = companies.map(function (company) {
        return { value: company.id, label: company.name };
    });

    function submit(e) {
        e.preventDefault();
        put(route("camps.update", camp));
    }
    return (
        <MainLayout user={auth.user} pageTitle={"Edit Camp"}>
            <div className="mb-5">
                <Link
